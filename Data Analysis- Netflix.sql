...
FROM [AdventureWorksDW2022].[dbo].[credits]
WHERE role = 'actor'
GROUP BY name
...
FROM [AdventureWorksDW2022].[dbo].[credits] AS c 
ON t.id = c.id
WHERE t.type = 'Movie' 
...
c.name AS director
FROM [AdventureWorksDW2022].[dbo].[titles] AS t
JOIN [AdventureWorksDW2022].[dbo].[credits] AS c 
ON t.id = c.id
WHERE t.type = 'Movie' 
...
FROM [AdventureWorksDW2022].[dbo].[credits] AS c
JOIN [AdventureWorksDW2022].[dbo].[titles] AS t 
ON c.id = t.id
WHERE c.role = 'actor'
